    ...
    while True:
        ...
        if not port_data:
            ...
            for port in port_data:
                ...
                # ---- Health Score Calculation (RPCLI) ----
                # Formula:
                # Health = 100 
                #          - (retrans * 10) 
                #          - (max(0, 100 - succ_rate) if syn > 0 else 0)
                #          - (max(0, avg_rtt - 50000) / 1000)  (Penalty if RTT > 50ms)
                # Max 100, Min 0
                
                health_score = 100.0
                
                # 1. Retransmission Penalty
                health_score -= (retrans * 10.0)
                
                # 2. Success Rate Penalty
                if syn > 0:
                    health_score -= (100.0 - succ_rate)
                
                # 3. Latency Penalty (Baseline 50ms/50000us)
                if avg_rtt > 50000:
                    health_score -= ((avg_rtt - 50000.0) / 1000.0)
                ...
                if health_score < ALERT_THRESHOLD:
                    subject = f"[Alert] RPCLI Health Low"
                    content = (f"Time: {timestamp}\n"
                               f"Port: {port}\n"
                               f"Health Score: {health_score:.2f} / 100\n"
                               f"Details:\n"
                               f" - Success Rate: {succ_rate:.2f}%\n"
                               f" - Avg RTT: {avg_rtt:.2f} us\n"
                               f" - Retransmissions: {retrans}\n"
                               f" - SYN Recv: {syn}, EST: {est}")
                    print(color_wrap(f"!!! Health Alert Triggered: {health_score:.2f} !!!", RED))
                    # Send email (async or simple call)
                    try:
                        email_alert.send_alert(subject, content)
                    except Exception as e:
                ...
                try:
                    ...
                    # Ignore write errors to avoid crashing monitor
                    pass
            # -------------------------
