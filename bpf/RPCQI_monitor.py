...
while (1):
    ...
    if args.dist:
        ...
        if total_conns == 0:
            ...
            # -------- Health Score & Alert Logic --------
            # Formula: Start at 100. Deduct points for bad states duration.
            # Assuming duration is in ms or us (label variable).
            
            health_score = 100.0
            # Penalize Loss heavily
            health_score -= (avg_loss * 5.0)
            # Penalize CWR (Congestion Window Reduced)
            health_score -= (avg_cwr * 2.0)
            # Penalize Recovery
            health_score -= (avg_recover * 2.0)
            # Penalize Disorder (out of order packets)
            health_score -= (avg_disorder * 1.0)
            
            if health_score < 0:
                health_score = 0.0
                
            print("  Health Score: %.2f" % health_score)

            if health_score < 70.0:
                alert_subject = "RPCQI Health Alert: Score %.2f" % health_score
                alert_body = (
                    "RPCQI Health Score is low: %.2f\n\n"
                    "Metrics (Average Duration per Connection):\n"
                    "- Loss: %.2f %s\n"
                    "- CWR: %.2f %s\n"
                    "- Recovery: %.2f %s\n"
                    "- Disorder: %.2f %s\n"
                    ...
                    "Threshold: 70.0\n"
                    "Time: %s\n"
                ) % (health_score, avg_loss, label, avg_cwr, label, 
                     avg_recover, label, avg_disorder, label, avg_open, label, timestamp)
                
                print("  [Alert] Health score below 70.0, sending email...")
                try:
                    email_alert.send_alert(alert_subject, alert_body)
                    print("  [Alert] Email sent successfully.")
                except Exception as e:
                    print("  [Alert] Failed to send email: %s" % e)
    ipv4_stat.clear()
    ipv6_stat.clear()
    countdown -= 1
